...
dataOrder.then(async (responseData) => {
    ...
    for (let m = 0; m < registeredProducts.length; m++){
        let totalPriceProduct = registeredProducts[m].prix;
        console.log(totalPriceProduct);
        totalPriceCalc += Number.parseFloat(totalPriceProduct);
        console.log((Math.round(totalPriceCalc * 100) / 100).toFixed(2));
    };

    //total prix commande
    let total = (totalPriceCalc * 100) ;
